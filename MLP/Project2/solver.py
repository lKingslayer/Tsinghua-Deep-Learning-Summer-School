...
class Solver(object):
    ...
    def build_model(cfg):
        model = Network()
        model.add(FCLayer(784, 256, act_function=cfg['activation_function']))
        model.add({'relu': ReLULayer, 'sigmoid': SigmoidLayer}[cfg['activation_function']]())
        model.add(FCLayer(256, 10, act_function=cfg['activation_function']))
        return model
